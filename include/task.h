...
namespace com
{
    ...
    // TODO: add possibility to name task, so it can by track using name or id
    class task
    {
    private:
        task_t t;
    ...
    public:
        explicit task(task_t &&t_, std::chrono::milliseconds interval_, std::size_t id_, repeat_t repeat_, std::chrono::seconds startup_delay_ = std::chrono::seconds(0)) : t(std::move(t_)), interval(interval_), id(id_), repeat(repeat_), next_time_call(std::chrono::floor<std::chrono::milliseconds>(std::chrono::steady_clock::now()) - interval_ + startup_delay_) {}
